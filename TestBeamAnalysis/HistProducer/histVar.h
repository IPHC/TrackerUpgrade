...
void createHist()
{
   tr = new TTree("DUT","DUT");
   tr->Branch("hits0",&hits0);
   tr->Branch("hits1",&hits1);
   tr->Branch("dutEvt",&dutEvt,"dutEvt/I");
   ...
   //occupancy
   hname = "h_occ";
   h_occ = new TH1D(hname.c_str(),hname.c_str(),8,0,8);
   h_occ->Sumw2();
   
   // dx for all hits
   hname = "h_s_dx";
   h_s_dx = new TH1D(hname.c_str(),hname.c_str(),60,-30.5,30.5);
   h_s_dx->Sumw2();
